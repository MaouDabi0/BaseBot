module.exports = {
  ...
  run: async (conn, message, { isPrefix }) => {
    ...
    if (!targetUser) return conn.sendMessage(chatId, { text: `❌ Gunakan format:\n${prefix}kick @user atau reply pesan target.` });

    if (groupAdmins.includes(targetUser)) return conn.sendMessage(chatId, { text: "❌ Tidak bisa mengeluarkan admin grup!" });

    await conn.groupParticipantsUpdate(chatId, [targetUser], "remove")
      .then(() => conn.sendMessage(chatId, { text: `✅ Berhasil mengeluarkan @${targetUser.split("@")[0]}`, mentions: [targetUser] }))
      .catch(() => conn.sendMessage(chatId, { text: "❌ Gagal mengeluarkan anggota. Pastikan bot adalah admin!" }));
  }
};