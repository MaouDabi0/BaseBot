...
const getMenuText = (sender) => {
  ...
  for (const [pluginName, plugin] of Object.entries(global.plugins)) {
    let category = plugin.tags || "Other Menu"; // Default kategori jika tidak ada `tags`
    if (!config.pluginCategories[category]) category = "Other Menu"; // Jika tidak terdaftar di `config.json`
    if (!categorizedCommands[category]) categorizedCommands[category] = [];
    categorizedCommands[category].push(pluginName);
  }

  // Menampilkan perintah dalam kategori
